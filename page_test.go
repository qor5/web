...
var mountCases = []struct {
...
}{
	{
		...
		expected: `
<div>
	<a href="#" v-on:click='plaid().vars(vars).locals(locals).form(plaidForm).eventFunc("bookmark").go()'>xgb123</a>
	<a href="#" v-on:blur='alert(1); plaid().vars(vars).locals(locals).form(plaidForm).fieldValue("Text1", $event).eventFunc("doIt").go()'>hello</a>
</div>
`,
	},
	{
